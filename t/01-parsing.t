...
diag("Testing Model Parsing for ModelCitizen $ModelCitizen::VERSION, Perl $], $^X");

# Filepath to our sample model
my $modelFilepath = 'samples/JJJ';

# Construct a list of files to munge and log file details
diag("Opening data model from [$modelFilepath]...");
my @inputFiles   = ModelCitizen::buildPackageFileList($modelFilepath, '.xml .local');
my $inputFileCnt = @inputFiles;
diag("$inputFileCnt model files in list.");
ok($inputFileCnt >= 4);    # Number of relevant files in our sample model
...
ok($loadedCnt >= 4);                                               # Make sure we loaded what we expected
...
diag("Loading STORES table from file: [$storesTablePath].");
...
diag("Loading S_SL_FK foreign key from file: [$S_SL_FKPath].");
...
# Let's test the enrichment
ModelCitizen::enrichModelFiles($loaded, []);

# Let's test the SQL Generation
ModelCitizen::getSQL($loaded);

# Let's test the SQL for the STORES table
my $storesTableWithSQL = first { $_->{name} eq "STORES" } @$loaded;
